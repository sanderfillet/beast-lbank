...
class DomainClient:
    ...
    async def request_metastats(self, get_opts, account_id: str):
        ...
        account_cache = self._account_cache[account_id]
        try:
            region_settings = account_cache['regions'][account_cache['regionIndex']]
            opts = get_opts(f'https://metastats-api-v1.{region_settings["region"]}.{self._url_cache["domain"]}',
                            region_settings['id'])
    ...
    async def _update_host(self):
        if not self._url_cache or self._url_cache['lastUpdated'] < datetime.now().timestamp() - 60 * 10:
            url_settings = await self._http_client.request_with_failover({
    ...
    async def _update_account_host(self, account_id: str):
        if account_id not in self._account_cache or self._account_cache[account_id]['lastUpdated'] < \
