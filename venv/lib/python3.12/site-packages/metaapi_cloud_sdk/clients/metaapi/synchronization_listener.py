...
class SynchronizationListener(ABC):
    ...
    async def on_positions_updated(
        self, instance_index: str, positions: MetatraderPosition, removed_positions_ids: List[str]
    ):
        """Invoked when MetaTrader positions are updated.

        Args:
            instance_index: Index of an account instance connected.
            positions: Updated MetaTrader position.
            removed_positions_ids: Removed positions ids.
        Returns:
            A coroutine which resolves when the asynchronous event is processed.
        """
        pass
