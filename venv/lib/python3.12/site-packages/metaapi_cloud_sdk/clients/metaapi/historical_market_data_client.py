...
class HistoricalMarketDataClient(MetaApiClient):
    def __init__(self, http_client: HttpClient, domain_client: DomainClient):
        """Initializes historical market data API client instance.
    ...
    async def get_historical_candles(
        self, account_id: str, region: str, symbol: str, timeframe: str, start_time: datetime = None, limit: int = None
        ...
        candles = await self._http_client.request(opts, 'get_historical_candles')
        candles = candles or []
        for c in candles:
            c['time'] = date(c['time'])
        return candles
    ...
    async def get_historical_ticks(
        ...
        ticks = await self._http_client.request(opts, 'get_historical_ticks')
        ticks = ticks or []
        for t in ticks:
            t['time'] = date(t['time'])
        return ticks
