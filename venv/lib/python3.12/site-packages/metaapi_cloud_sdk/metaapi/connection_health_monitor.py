...
from ..clients.metaapi.synchronization_listener import SynchronizationListener, HealthStatus
...
class ConnectionHealthMonitor(SynchronizationListener):
    ...
    def __init__(self, connection):
        ...
        self._quotes_healthy = False
        self._offset = 0
        self._min_quote_interval = 60
        self._update_quote_health_status_interval = asyncio.create_task(update_quote_health_job())
        self._measure_uptime_interval = asyncio.create_task(measure_uptime_job())
        self._server_health_status = {}
        self._uptime_reservoirs = {
    ...
    def stop(self):
        ...
        self._update_quote_health_status_interval.cancel()
        self._measure_uptime_interval.cancel()
    ...
    async def on_symbol_price_updated(self, instance_index: str, price: MetatraderSymbolPrice):
        ...
        try:
            ...
            self._logger.error(
                f'{self._connection.account.id}: Failed to update quote streaming health '
    ...
    async def on_health_status(self, instance_index: str, status: HealthStatus):
        ...
        """
        self._server_health_status[str(instance_index)] = status
    ...
    async def on_disconnected(self, instance_index: str):
        ...
        Returns:
             A coroutine which resolves when the asynchronous event is processed.
        """
        if str(instance_index) in self._server_health_status:
            del self._server_health_status[str(instance_index)]
    ...
    def server_health_status(self) -> HealthStatus:
        ...
        """
        result = None
        for s in self._server_health_status:
            if not result:
                result = s
            else:
                for field in list(s.keys()):
                    result[field] = result.get(field, s[field])
        return result or {}
    ...
    def health_status(self) -> ConnectionHealthStatus:
        ...
        status = {
            ...
            'quoteStreamingHealthy': self._quotes_healthy,
            'synchronized': self._connection.synchronized,
        }
        status['healthy'] = (
            status['connected']
            and status['connectedToBroker']
            and status['quoteStreamingHealthy']
    ...
    def uptime(self) -> dict:
        ...
        """
        uptime = {}
        for key in self._uptime_reservoirs:
            e: Reservoir = self._uptime_reservoirs[key]
    ...
    def _measure_uptime(self):
        try:
            for key in self._uptime_reservoirs:
                r = self._uptime_reservoirs[key]
                r.push_measurement(
                    100
                    if (
                        self._connection.terminal_state.connected
                        and self._connection.terminal_state.connected_to_broker
                        and self._connection.synchronized
                        and self._quotes_healthy
                    )
                    else 0
                )
        except Exception as err:
            self._logger.error(
                f'failed to measure uptime for account {self._connection.account.id} ' f'{string_format_error(err)}'
            )
    ...
    def _update_quote_health_status(self):
        try:
            ...
            for symbol in self._connection.subscribed_symbols:
                ...
                quote_sessions_list = specification.get('quoteSessions', [])
                quote_sessions = (
                    quote_sessions_list[days_of_week[str(day_of_week)]]
                    if days_of_week[str(day_of_week)] in quote_sessions_list
                    else []
                )
                for session in quote_sessions:
                    if session['from'] <= server_time <= session['to']:
                        in_quote_session = True
            self._quotes_healthy = (
                (not len(self._connection.subscribed_symbols))
                or (not in_quote_session)
                or ((datetime.now().timestamp() - self._priceUpdatedAt.timestamp()) < self._min_quote_interval)
            )
        except Exception as err:
            self._logger.error(
                f'failed to update quote streaming health status for account '
