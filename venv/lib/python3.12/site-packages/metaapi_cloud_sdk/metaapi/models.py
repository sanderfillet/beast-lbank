...
def date(date_time: Union[str, float, int]) -> datetime:
    ...
    if isinstance(date_time, float) or isinstance(date_time, int):
        ...
        max_date = datetime(3000, 1, 1).astimezone(pytz.utc)
        if isinstance(date_time, str) and date_time.startswith('+'):
            # this prevents errors if expirationTime year is >=10000
            return max_date
        else:
            return min(iso8601.parse_date(date_time), max_date)
