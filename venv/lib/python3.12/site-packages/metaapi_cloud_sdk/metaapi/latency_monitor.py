...
from .reservoir.statistical_reservoir import StatisticalReservoir
from ..clients.metaapi.latency_listener import (
    LatencyListener,
    ResponseTimestamps,
    SymbolPriceTimestamps,
    UpdateTimestamps,
    TradeTimestamps,
)
...
class LatencyMonitor(LatencyListener):
    ...
    def __init__(self):
        """Initializes latency monitor instance."""
        super().__init__()
        ...
        self._requestReservoirs = {'branch': True}
    ...
    async def on_response(self, account_id: str, type: str, timestamps: ResponseTimestamps):
        ...
        if 'serverProcessingStarted' in timestamps and 'serverProcessingFinished' in timestamps:
            server_latency = (
                timestamps['serverProcessingFinished'].timestamp() - timestamps['serverProcessingStarted'].timestamp()
            )
            self._save_measurement(self._requestReservoirs[type]['serverLatency'], server_latency)

        if (
            'clientProcessingStarted' in timestamps
            and 'clientProcessingFinished' in timestamps
            and 'serverProcessingStarted' in timestamps
            and 'serverProcessingFinished' in timestamps
        ):
            server_latency = (
                timestamps['serverProcessingFinished'].timestamp() - timestamps['serverProcessingStarted'].timestamp()
            )
            client_latency = (
                timestamps['clientProcessingFinished'].timestamp()
    ...
    async def on_symbol_price(self, account_id: str, symbol: str, timestamps: SymbolPriceTimestamps):
        ...
        if 'eventGenerated' in timestamps and 'serverProcessingStarted' in timestamps:
            broker_latency = (
                timestamps['serverProcessingStarted'].timestamp() - timestamps['eventGenerated'].timestamp()
            )
        ...
        if 'serverProcessingFinished' in timestamps and 'clientProcessingFinished' in timestamps:
            client_latency = (
                timestamps['clientProcessingFinished'].timestamp() - timestamps['serverProcessingFinished'].timestamp()
            )
    ...
    async def on_trade(self, account_id: str, timestamps: TradeTimestamps):
        ...
        if 'clientProcessingStarted' in timestamps and 'serverProcessingStarted' in timestamps:
            client_latency = (
                timestamps['serverProcessingStarted'].timestamp() - timestamps['clientProcessingStarted'].timestamp()
            )
        ...
        if 'serverProcessingStarted' in timestamps and 'tradeStarted' in timestamps:
            server_latency = timestamps['tradeStarted'].timestamp() - timestamps['serverProcessingStarted'].timestamp()
        ...
        if 'tradeStarted' in timestamps and 'tradeExecuted' in timestamps:
            broker_latency = timestamps['tradeExecuted'].timestamp() - timestamps['tradeStarted'].timestamp()
    ...
    def _construct_latencies_recursively(self, reservoirs):
        ...
        for key in reservoirs:
            ...
            if key == 'branch':
                continue
            result[key] = (
                self._construct_latencies_recursively(e)
                if ('branch' in e and e['branch'])
                else {
