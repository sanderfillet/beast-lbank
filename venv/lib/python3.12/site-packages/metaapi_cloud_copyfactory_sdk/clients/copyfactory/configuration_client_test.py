...
class TestWebhooks:
    ...
    @pytest.mark.asyncio
    async def test_retrieve_webhooks_from_api_with_infinite_pagination(self):
        """Should retrieve webhooks from API with infinite pagination"""
        expected = [
            {
                '_id': 'id1',
                'createdAt': date(10),
        ...
        domain_client.request_copyfactory = AsyncMock(return_value=expected)
        actual = await copy_factory_client.get_webhooks_with_infinite_scroll_pagination('strategy1', {
            'limit': 100,
            'offset': 200
        })
        ...
        domain_client.request_copyfactory.assert_called_with(
            {
                ...
                'method': 'GET',
                'headers': {'auth-token': token},
                'params': {'paginationStyle': 'infiniteScroll', 'limit': 100, 'offset': 200},
            }, True
        )

    @pytest.mark.asyncio
    async def test_retrieve_webhooks_from_api_with_classic_pagination(self):
        """Should retrieve webhooks from API with classic pagination"""
        expected = {
            'count': 2,
            'items': [
                {
        ...
        domain_client.request_copyfactory = AsyncMock(return_value=expected)
        actual = await copy_factory_client.get_webhooks_with_classic_scroll_pagination('strategy1', {
            'limit': 100,
            'offset': 200
        })
        assert actual == expected
        domain_client.request_copyfactory.assert_called_with(
            {
                ...
                'method': 'GET',
                'headers': {'auth-token': token},
                'params': {'paginationStyle': 'classic', 'limit': 100, 'offset': 200},
            }, True
        )

    @pytest.mark.asyncio
    async def test_create_webhook_via_api(self):
        """Should create webhook via API"""
        webhook = {
            'magic': 100,
            'symbolMapping': [{'from': 'symbol1', 'to': 'symbol2'}]
        }
        domain_client.request_copyfactory = AsyncMock(return_value=webhook)
        actual = await copy_factory_client.create_webhook('strategy1', webhook)
        assert actual == webhook
        domain_client.request_copyfactory.assert_called_with(
            {
                'url': '/users/current/configuration/strategies/strategy1/webhooks',
                'method': 'POST',
    ...
    @pytest.mark.asyncio
    async def test_update_webhook_via_api(self):
        """Should update webhook via API"""
        webhook = {
            'magic': 100,
            'symbolMapping': [{'from': 'symbol1', 'to': 'symbol2'}]
        }
        domain_client.request_copyfactory = AsyncMock(return_value=webhook)
        await copy_factory_client.update_webhook('strategy1', 'webhook1', webhook)
        domain_client.request_copyfactory.assert_called_with(
            {
    ...
    @pytest.mark.asyncio
    async def test_delete_webhook_via_api(self):
        """Should delete webhook ia API"""
        domain_client.request_copyfactory = AsyncMock()
        await copy_factory_client.delete_webhook('strategy1', 'webhook1')
        domain_client.request_copyfactory.assert_called_with(
            {
                'url': '/users/current/configuration/strategies/strategy1/webhooks/webhook1',
                'method': 'DELETE',
